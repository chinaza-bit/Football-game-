(() => {
  // Best-effort: some embedded/PWA contexts allow orientation lock without a
  // gesture. This silently does nothing in a normal browser tab — the
  // tap-to-enter gate below is what actually works there, since browsers
  // require a user gesture before granting fullscreen/orientation-lock.
  ...
  // ============================================================
  // TAP-TO-START GATE
  // Browsers require a user gesture before fullscreen/orientation-lock
  // requests are allowed, so we can't force landscape the instant the
  // page loads — this gate captures that first tap and uses it.
  // ============================================================
  document.getElementById('enterBtn').addEventListener('click', () => {
    ...
    try {
      ...
      const fsResult = req ? req.call(el) : Promise.resolve();
      Promise.resolve(fsResult).finally(() => {
        try {
          if (screen.orientation && screen.orientation.lock) {
            screen.orientation.lock('landscape').catch(() => {});
          }
        } catch (e) { /* not supported on this browser (e.g. iOS Safari) — rotate prompt is the fallback */ }
      });
    } catch (e) { /* no-op */ }
  ...
  function startGame(){
    ...
    let linesmanTop = { x:W/2, y:PITCH_TOP-9 };
    let linesmanBottom = { x:W/2, y:PITCH_BOTTOM+9 };
    let lastTouchSide = 'home';
    ...
    window.addEventListener('keydown', e => {
      ...
      if (e.key === ' ') { e.preventDefault(); doAction('shoot'); }
      if (k === 'j') doAction('shortPass');
      if (k === 'k') doAction('longPass');
    });
    ...
    // ---------- input: virtual joystick (any-direction analog movement) ----------
    ...
    (function setupJoystick(){
      ...
      stick.addEventListener('pointermove', e => {
        if (activeId === e.pointerId) setFromEvent(e.clientX, e.clientY);
      });
      stick.addEventListener('pointerup', e => { if (activeId === e.pointerId) reset(); });
      stick.addEventListener('pointercancel', () => reset());
    })();

    // ---------- input: action buttons ----------
    document.getElementById('shotBtn').addEventListener('click', () => doAction('shoot'));
    document.getElementById('shortPassBtn').addEventListener('click', () => doAction('shortPass'));
    document.getElementById('longPassBtn').addEventListener('click', () => doAction('longPass'));
    const runBtn = document.getElementById('runBtn');
    runBtn.addEventListener('pointerdown', () => { sprintHeld = true; });
    runBtn.addEventListener('pointerup', () => { sprintHeld = false; });
    runBtn.addEventListener('pointercancel', () => { sprintHeld = false; });
    runBtn.addEventListener('pointerleave', () => { sprintHeld = false; });
    ...
    function flashStatus(t){
      ...
      setTimeout(()=>{ if(!paused) statusEl.textContent = 'Move + Space to shoot'; }, 900);
    }
    ...
    function teamName(side){ return side==='home' ? HOME_NAME : AWAY_NAME; }

    const ACTION_PROFILES = {
      shortPass: { power:4.6,  vz:1.2, spin:0.04, label:'Short pass' },
      longPass:  { power:8,    vz:5,   spin:0.08, label:'Long ball!' },
      shoot:     { power:8.5,  vz:3.2, spin:0.16, label:'Shot!' },
    };

    function doAction(type){
      if (paused || refereeStoppage) return;
      const p = controlled;
      if (dist(p, ball) >= PLAYER_R+BALL_R+16 || ball.z >= 22) return;
      const profile = ACTION_PROFILES[type] || ACTION_PROFILES.shoot;
      ...
      ball.vx = dirX*profile.power; ball.vy = dirY*profile.power;
      ball.vz = profile.vz + Math.random()*1.5;              // lofts the ball into an arc
      ball.spin = (Math.random()*2-1) * profile.spin;         // curls the flight path (banana kick/pass)
      lastTouchSide = p.side;

      if (type === 'shoot'){
        stats[p.side].shots++;
        const goalX = p.side === 'home' ? W : 0;
        ...
        if (Math.acos(cosAngle) < 0.4) stats[p.side].onTarget++;
      }
      flashStatus(profile.label);
    }
    ...
    function goalScored(side){
      ...
      // celebration/reaction: scoring side is happy, conceding side is dejected —
      // and the broadcast camera pushes in on the goalmouth for the celebration
      const now = performance.now();
      const scorers = side==='home' ? homeTeam : awayTeam;
      const concede = side==='home' ? awayTeam : homeTeam;
      scorers.forEach(pl => { pl.mood='happy'; pl.moodUntil = now+3500; });
      concede.forEach(pl => { pl.mood='sad'; pl.moodUntil = now+2200; });
      const goalX = side==='home' ? W-10 : 10;
      focusCamera(goalX, 1.9, 1800);

      paused = true;
      lastTouchSide = side === 'home' ? 'away' : 'home'; // conceding side restarts play
        ...
        for (const b of awayTeam){
          ...
          if (d < PLAYER_R*1.3 && dist(a,ball) < 40 && dist(b,ball) < 40){
            ...
            if (Math.random() < chance){
              ...
              // the two players involved react — this is what "human" reactions look like
              const now = performance.now();
              const fouler = foulingSide === 'home' ? a : b;
              const fouled = foulingSide === 'home' ? b : a;
              fouler.mood = 'angry'; fouler.moodUntil = now + 2600;
              fouled.mood = 'sad';   fouled.moodUntil = now + 2400;

              refereeStoppage = true;
              const spotX = (a.x+b.x)/2, spotY = (a.y+b.y)/2;
              referee.targetX = spotX; referee.targetY = spotY;

              // broadcast-style highlight: zoom in on the incident, then a short
              // VAR review, then the confirmed decision — before play resumes
              focusCamera(spotX, 2.15, 2700);
              showBanner('🟨 Referee whistle! Foul!', 900);
              setTimeout(() => showBanner('📺 Checking VAR...', 1300), 900);
              setTimeout(() => {
                showBanner('✅ Free kick to ' + teamName(fouledSide) + '.' + cardText, 1300);
              }, 2200);
              setTimeout(() => {
                refereeStoppage = false;
                ball.x = spotX; ball.y = spotY; ball.vx = 0; ball.vy = 0; ball.vz = 0; ball.spin = 0;
                lastTouchSide = fouledSide;
              }, 2700);
              foulCooldown = 340;
              return;
            }
    ...
    function aiMoveTeam(team, isUserTeam){
      ...
      for (const p of team){
        ...
        let targetX, targetY;
        const ballIsNear = dist(p,ball) < 95;
        if (p.role === 'GK'){
          ...
          // stronger pull back toward formation shape keeps the team tighter
          // and less scattered across the pitch
          const pullX = (ball.x-p.baseX)*0.22;
          const pullY = (ball.y-p.baseY)*0.22;
          targetX = p.baseX+pullX; targetY = p.baseY+pullY;
          if (ballIsNear){ targetX = ball.x - attackDir*6; targetY = ball.y; }
        }
        const dx = targetX-p.x, dy = targetY-p.y;
        const d = Math.hypot(dx,dy)||1;
        const speed = (ballIsNear?1.9:1.25) * DIFF;
    ...
    function moveControlledPlayer(){
      ...
      // virtual joystick overrides/combines with keyboard input
      if (Math.hypot(joystickVec.x, joystickVec.y) > 0.12){
        dx = joystickVec.x; dy = joystickVec.y;
      }
      const BASE_SPEED = 2.3;
      const MAX_SPEED = sprintHeld ? BASE_SPEED*1.4 : BASE_SPEED;
      ...
      // (or toward zero via friction when no input) instead of snapping, so the
      // player can move fluidly in any direction — N, S, E, W, or any angle between
    ...
    function awardThrowIn(edge){
      // edge: 'top' or 'bottom' — whichever sideline the ball crossed
      const throwSide = lastTouchSide === 'home' ? 'away' : 'home';
      refereeStoppage = true;
      const outX = clamp(ball.x, 24, W-24);
      const outY = edge === 'top' ? PITCH_TOP+11 : PITCH_BOTTOM-11;
      ball.vx=0; ball.vy=0; ball.vz=0; ball.spin=0; ball.x=outX; ball.y=outY;
      showBanner('🚩 Throw-in to ' + teamName(throwSide), 1500);
      setTimeout(() => {
        refereeStoppage = false;
        const inward = edge === 'top' ? 1 : -1;
        ball.vy = inward * (2.2 + Math.random()*0.8);
        ball.vx = (Math.random()-0.5) * 1.8;
        ball.vz = 1.4 + Math.random()*0.8;
        lastTouchSide = throwSide;
      }, 1200);
    }

    function awardCorner(attackingSide, edge){
      // attackingSide takes the kick from the corner arc nearest the edge (top/bottom)
      // on the goal line they're attacking
      refereeStoppage = true;
      const cornerX = attackingSide === 'home' ? W-12 : 12;
      const cornerY = edge === 'top' ? PITCH_TOP+9 : PITCH_BOTTOM-9;
      ball.x = cornerX; ball.y = cornerY; ball.vx=0; ball.vy=0; ball.vz=0; ball.spin=0;
      focusCamera(cornerX, 1.65, 1300);
      showBanner('🚩 Corner Kick to ' + teamName(attackingSide), 1500);
      setTimeout(() => {
        refereeStoppage = false;
        const boxX = attackingSide === 'home' ? W-95 : 95;
        const boxY = PITCH_TOP + PITCH_H/2;
        const dx = boxX-ball.x, dy = boxY-ball.y;
        const len = Math.hypot(dx,dy) || 1;
        ball.vx = (dx/len)*4.6; ball.vy = (dy/len)*4.6; ball.vz = 5.5 + Math.random()*1.5;
        lastTouchSide = attackingSide;
      }, 1200);
    }

    function awardGoalKick(defendingSide){
      refereeStoppage = true;
      const gx = defendingSide === 'home' ? 40 : W-40;
      ball.x = gx; ball.y = PITCH_TOP+PITCH_H/2; ball.vx=0; ball.vy=0; ball.vz=0; ball.spin=0;
      showBanner('Goal Kick — ' + teamName(defendingSide), 1300);
      setTimeout(() => {
        refereeStoppage = false;
        const dir = defendingSide === 'home' ? 1 : -1;
        ball.vx = dir * (4.5 + Math.random()*0.8);
        ball.vy = (Math.random()-0.5)*1.8;
        ball.vz = 4.5 + Math.random()*1.5;
        lastTouchSide = defendingSide;
      }, 1100);
    }
    ...
    function updateBallPhysics(){
      for (const p of awayTeam){
        if (dist(p,ball) < PLAYER_R+BALL_R+6 && Math.hypot(ball.vx,ball.vy) < 3 && ball.z < 14){
          ...
          ball.vx = dx*4*DIFF + (Math.random()-0.5)*1;
          ball.vy = dy*4*DIFF + (Math.random()-0.5)*1;
          ball.vz = 2.4 + Math.random()*2;
      ...
      // sideline crossed -> throw-in (instead of bouncing back into play)
      if (ball.y < PITCH_TOP+BALL_R){ awardThrowIn('top'); return; }
      if (ball.y > PITCH_BOTTOM-BALL_R){ awardThrowIn('bottom'); return; }

      const goalTop = PITCH_TOP+PITCH_H/2-GOAL_W/2, goalBottom = PITCH_TOP+PITCH_H/2+GOAL_W/2;
      const exitEdge = ball.y < PITCH_TOP+PITCH_H/2 ? 'top' : 'bottom';
      ...
      if (ball.x < BALL_R+2){
        if (ball.y>goalTop && ball.y<goalBottom){
          ...
          // ball went out over home's own goal line, not a goal
          if (lastTouchSide === 'home') awardCorner('away', exitEdge);
          else awardGoalKick('home');
          return;
        }
      }
      if (ball.x > W-BALL_R-2){
        if (ball.y>goalTop && ball.y<goalBottom){
          ...
          // ball went out over away's own goal line, not a goal
          if (lastTouchSide === 'away') awardCorner('home', exitEdge);
          else awardGoalKick('away');
          return;
        }
      }
    }
    ...
    function updateReferee(){
      ...
      // linesmen shadow the ball along their touchline, roughly level with play
      linesmanTop.x += (clamp(ball.x, 20, W-20) - linesmanTop.x) * 0.05;
      linesmanBottom.x += (clamp(ball.x, 20, W-20) - linesmanBottom.x) * 0.05;
    }

    // ---------- pseudo-3D projection + broadcast camera ----------
    ...
    // A camera then pans/zooms horizontally to follow the ball, the way a real
    // TV broadcast camera tracks play along the length of the pitch while keeping
    // the full width of the pitch in frame.
    const CENTER_X = W/2;
    const FAR_SCALE = 0.72, NEAR_SCALE = 1.0;
    const CAM_ZOOM = 1.35;
    let camX = W/2;
    let camZoomCurrent = CAM_ZOOM;
    let camOverride = null; // {x, zoom, until} — set during highlight moments (fouls, goals, corners)

    function focusCamera(x, zoom, durationMs){
      camOverride = { x, zoom, until: performance.now() + durationMs };
    }

    function updateCamera(){
      const now = performance.now();
      let targetX, targetZoom;
      if (camOverride && now < camOverride.until){
        targetX = camOverride.x; targetZoom = camOverride.zoom;
      } else {
        camOverride = null;
        targetX = ball.x; targetZoom = CAM_ZOOM;
      }
      const panEase = camOverride ? 0.12 : 0.045; // snap faster into a highlight, ease slower on normal follow
      camX += (targetX - camX) * panEase;
      camZoomCurrent += (targetZoom - camZoomCurrent) * 0.08;
      const halfViewWorld = (W/2) / camZoomCurrent;
      camX = clamp(camX, halfViewWorld, W - halfViewWorld);
    }
    ...
    function projX(x, y){
      const warped = CENTER_X + (x-CENTER_X) * scaleAtY(y);
      // camera pan + zoom applied on top of the perspective warp
      return (warped - camX) * camZoomCurrent + CENTER_X;
    }
    // effective size scale for entities = perspective distance scale * camera zoom
    function entityScale(y){ return scaleAtY(y) * camZoomCurrent; }
    ...
    function drawPitch(){
      ...
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(projX(W/2,PITCH_TOP+PITCH_H/2), PITCH_TOP+PITCH_H/2, 3*entityScale(PITCH_TOP+PITCH_H/2), 0, Math.PI*2);
      ...
      // corner arcs
      ctx.lineWidth = 1.5;
      [[4,PITCH_TOP+4],[4,PITCH_BOTTOM-4],[W-4,PITCH_TOP+4],[W-4,PITCH_BOTTOM-4]].forEach(([cx,cy]) => {
        ctx.beginPath();
        ctx.arc(projX(cx,cy), cy, 6*entityScale(cy), 0, Math.PI*2);
        ctx.stroke();
      });
    }

    function drawPerson(p, bodyColor, shortsColor, isControlled, num, skin, kitAccent){
      const s = entityScale(p.y);
      const px = projX(p.x, p.y);
      const facing = Math.atan2(p.faceY||0, p.faceX||1);
      const moveSpeed = Math.hypot(p.vx||0, p.vy||0);
      // running animation: legs/arms swing based on accumulated stride phase
      p._stride = (p._stride||0) + Math.min(moveSpeed, 4) * 0.09;
      const swing = Math.sin(p._stride) * Math.min(1, moveSpeed/1.2);
      ...
      // ground shadow
      ctx.beginPath();
      ctx.ellipse(0, PLAYER_R*0.85, PLAYER_R*0.85, PLAYER_R*0.32, 0, 0, Math.PI*2);
      ctx.fillStyle = 'rgba(0,0,0,0.3)'; ctx.fill();

      // legs (animated stride — one forward, one back)
      ctx.fillStyle = '#111';
      ctx.fillRect(-5.5+swing*2, 8+Math.abs(swing)*0.5, 3.4, 3.2); // boots
      ctx.fillRect(2+swing*-2, 8+Math.abs(swing)*0.5, 3.4, 3.2);
      ctx.fillStyle = shortsColor;
      ctx.fillRect(-5.5+swing*2, 1, 3.6, 8);
      ctx.fillRect(2+swing*-2, 1, 3.6, 8);
      ctx.fillStyle = '#1a1a1a'; // socks
      ctx.fillRect(-5.5+swing*2, 6, 3.6, 3);
      ctx.fillRect(2+swing*-2, 6, 3.6, 3);

      // torso (jersey) — subtle gradient for rounded, less-flat volume
      const torsoGrad = ctx.createLinearGradient(-7,-8,7,4);
      torsoGrad.addColorStop(0, shade(bodyColor, 28));
      torsoGrad.addColorStop(0.55, bodyColor);
      torsoGrad.addColorStop(1, shade(bodyColor, -22));
      ctx.beginPath();
      ctx.moveTo(-7.5,-9); ctx.lineTo(7.5,-9); ctx.lineTo(6.5,4); ctx.lineTo(-6.5,4); ctx.closePath();
      ctx.fillStyle = torsoGrad; ctx.fill();
      ctx.strokeStyle = 'rgba(0,0,0,0.3)'; ctx.lineWidth=0.8; ctx.stroke();
      if (kitAccent){ ctx.fillStyle = kitAccent; ctx.fillRect(-7.5,-9,15,2.2); } // collar/trim

      // arms (swing opposite the legs)
      ctx.fillStyle = shade(bodyColor, -8);
      ctx.fillRect(-9.5-swing*1.5, -7, 3, 8);
      ctx.fillRect(6.5+swing*1.5, -7, 3, 8);
      ctx.fillStyle = skin || '#e0ac7a';
      ctx.fillRect(-9.5-swing*1.5, 0, 3, 2.4);
      ctx.fillRect(6.5+swing*1.5, 0, 3, 2.4);
      ...
      // head with simple shading + hair
      const headGrad = ctx.createRadialGradient(-1.5,-15,1,0,-13,5.2);
      headGrad.addColorStop(0, shade(skin||'#e0ac7a', 20));
      headGrad.addColorStop(1, shade(skin||'#e0ac7a', -12));
      ctx.beginPath();
      ctx.arc(0, -13, 5, 0, Math.PI*2);
      ctx.fillStyle = headGrad; ctx.fill();
      ctx.strokeStyle='rgba(0,0,0,0.25)'; ctx.stroke();
      ctx.fillStyle = '#2b1c12';
      ctx.beginPath(); ctx.arc(0,-15.5,5,Math.PI,Math.PI*2); ctx.fill(); // hair cap

      // face: eyes, nose, mouth — expression driven by the player's current mood
      const mood = (p.moodUntil && performance.now() < p.moodUntil) ? p.mood : 'neutral';
      // eyes (whites + pupils looking in the facing direction)
      const lookX = Math.cos(facing)*0.5, lookY = Math.sin(facing)*0.5;
      ctx.fillStyle = '#fff';
      ctx.beginPath(); ctx.ellipse(-1.7,-13.6,1.05,0.75,0,0,Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.ellipse(1.7,-13.6,1.05,0.75,0,0,Math.PI*2); ctx.fill();
      ctx.fillStyle = '#1a1208';
      ctx.beginPath(); ctx.arc(-1.7+lookX,-13.6+lookY,0.45,0,Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.arc(1.7+lookX,-13.6+lookY,0.45,0,Math.PI*2); ctx.fill();
      // eyebrows (angled down for anger, raised for happy)
      ctx.strokeStyle = 'rgba(30,18,10,0.8)'; ctx.lineWidth = 0.55;
      if (mood === 'angry'){
        ctx.beginPath(); ctx.moveTo(-2.6,-15.1); ctx.lineTo(-0.8,-14.4); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(2.6,-15.1); ctx.lineTo(0.8,-14.4); ctx.stroke();
      } else {
        ctx.beginPath(); ctx.moveTo(-2.6,-14.9); ctx.lineTo(-0.8,-15.1); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(2.6,-14.9); ctx.lineTo(0.8,-15.1); ctx.stroke();
      }
      // nose
      ctx.strokeStyle = 'rgba(0,0,0,0.3)'; ctx.lineWidth = 0.5;
      ctx.beginPath(); ctx.moveTo(0,-13); ctx.lineTo(0.5,-11.6); ctx.stroke();
      // mouth — smile / frown / neutral
      ctx.strokeStyle = '#5a2f22'; ctx.lineWidth = 0.6;
      ctx.beginPath();
      if (mood === 'happy'){
        ctx.arc(0,-11.4,1.3,0.15*Math.PI,0.85*Math.PI);
      } else if (mood === 'sad' || mood === 'angry'){
        ctx.arc(0,-9.7,1.2,1.2*Math.PI,1.8*Math.PI);
      } else {
        ctx.moveTo(-1,-10.6); ctx.lineTo(1,-10.6);
      }
      ctx.stroke();
    ...
    function drawPlayer(p){
      ...
      drawPerson(p, p.color, shorts, p===controlled, p.num, null, '#ffffff');
    }
    function drawReferee(){
      drawPerson({x:referee.x, y:referee.y, faceX:1, faceY:0}, '#111111', '#111111', false, '');
    }
    function drawLinesman(lm){
      drawPerson({x:lm.x, y:lm.y, faceX:1, faceY:0}, '#e8e14e', '#111111', false, '');
      // small flag
      const s = entityScale(lm.y);
      const px = projX(lm.x, lm.y);
      ctx.save();
      ctx.translate(px, lm.y); ctx.scale(s,s);
      ctx.fillStyle = '#ffffff';
      ctx.fillRect(9, -18, 6, 4);
      ctx.strokeStyle = '#333'; ctx.lineWidth=0.6;
      ctx.beginPath(); ctx.moveTo(9,-9); ctx.lineTo(9,-18); ctx.stroke();
      ctx.restore();
    }
    function drawBall(){
      const s = entityScale(ball.y);
    ...
    function render(){
      ...
      entities.push({ y:linesmanTop.y, draw:()=>drawLinesman(linesmanTop) });
      entities.push({ y:linesmanBottom.y, draw:()=>drawLinesman(linesmanBottom) });
